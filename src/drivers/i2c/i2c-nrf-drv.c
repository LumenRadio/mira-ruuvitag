...
int
nrf_i2c_xfer_is_done()
{
    return m_xfer_done;
}

int
i2c_init(i2c_config_t* config)
{

    if (!i2c_is_initialized) {

        const nrf_drv_twi_config_t twi_config = { .scl = config->scl,
                                                  .sda = config->sda,
                                                  .frequency = NRF_DRV_TWI_FREQ_100K,
                                                  .interrupt_priority = APP_IRQ_PRIORITY_LOW,
                                                  .clear_bus_init = false };

        uint32_t err_code = nrf_drv_twi_init(&m_twi, &twi_config, twi_handler, NULL);
...
int
nrf_i2c_write(uint8_t I2C_slave_address, uint8_t* tx_buff, uint8_t len)
{
    m_xfer_done = false;
    waiting_process = PROCESS_CURRENT();
    uint32_t err_code = nrf_drv_twi_tx(&m_twi, I2C_slave_address, tx_buff, len, false);
...
int
nrf_i2c_read(uint8_t I2C_slave_address,
             uint8_t* tx_buff,
             uint8_t tx_len,
             ...
             uint8_t rx_len)
{
    ...
    nrf_drv_twi_xfer_desc_t p_xfer = { .type = NRF_DRV_TWI_XFER_TXRX,
