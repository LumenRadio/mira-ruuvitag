...
/*function prototype */
float sensor_sht31_calc_temp(
    uint16_t raw_value);
float sensor_sht31_calc_humid(
    uint16_t raw_value);
...
PROCESS_THREAD(sensor_shtc3_init, ev, data)
{
    ...
    nrf_i2c_read(SHTC3_I2C_SLAVE_ADDRESS,
        read_id,
        sizeof(read_id),
        values,
        sizeof(values));
...
PROCESS_THREAD(sensor_shtc3_sample, ev, data)
{
    ...
    static uint8_t wakeup_config[2] = {
        0x35, 0x17
    };

    static uint8_t measure_config[2] = {
        0x64, 0x58
    };
    static uint8_t sleep_command[2] = {
        0xB0, 0x98
    };
